...
// 👇 API BACKEND
const API_ADMIN_MENU = 'https://gustoweb.onrender.com/api/admin/AdminRestaurantMenu';
const API_ADMIN_TABLE = 'https://gustoweb.onrender.com/api/admin/AdminRestaurantTable';
const API_PROFILE = 'https://gustoweb.onrender.com/api/admin/AdminRestaurantProfile';
...
const RestaurantDetail = ({ restaurantId, onBack }) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (modalType === 'MENU') {
                const url = isEdit
                    ? `${API_ADMIN_MENU}/${restaurantId}/${formData.foodId}`
                    : `${API_ADMIN_MENU}/${restaurantId}`;
                isEdit ? await axios.put(url, payload) : await axios.post(url, payload);
            } else {
                const url = isEdit
                    ? `${API_ADMIN_TABLE}/${restaurantId}/${formData.tableId}`
                    : `${API_ADMIN_TABLE}/${restaurantId}`;
                isEdit ? await axios.put(url, payload) : await axios.post(url, payload);
            }
            alert('Thành công!');
            handleCloseModal();
                    ...
                    <div className={styles.formGroup}>
                        ...
                        <div className={styles.fileUploadWrapper}>
                            ...
                            <label htmlFor="fileInput" className={styles.fileLabel}>
                                {uploading ? <FaSpinner className={styles.spinner} /> : <FaCloudUploadAlt />}{' '}
            ...
            {modalType === 'MENU' ? (
                <>
                    ...
                    <div className={styles.detailItem}>
                        ...
                        <span className={styles.menuPrice}>{formData.price?.toLocaleString()} VNĐ</span>
                    </div>
                    <div className={styles.detailItem} style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                        <span className={styles.label}>Mô tả:</span>
                        <p className={styles.descText}>{formData.description}</p>
                    </div>
                </>
            ) : (
                <>
                    ...
                    <div className={styles.detailItem}>
                        ...
                        <span className={styles.value}>{formData.personNumber}</span>
                    </div>
                    <div className={styles.detailItem}>
                        <span className={styles.label}>Trạng thái:</span>
                        <span className={formData.status === 'Available' ? styles.tagGreen : styles.tagRed}>
                            {formData.status}
                        </span>
                    </div>
                </>
        ...
        <div className={styles.container}>
            ...
            <div className={styles.section}>
                <h2 className={styles.title}>
                    <FaInfoCircle /> Thông tin nhà hàng
                </h2>
                {profile && (
                    <div className={styles.profileContent}>
                        <div className={styles.avatarSection}>
                            {profile.avatarUrl ? (
                                ...
                                <div className={styles.placeholderAvatar}>🍽️</div>
                            )}
                        </div>
            ...
            <div className={styles.gridContainer}>
                ...
                <div className={styles.section}>
                    <div className={styles.sectionHeader}>
                        <h2 className={styles.title}>
                            <FaChair /> Bàn ({filteredTables.length})
                        </h2>
                        <button className={styles.addBtn} onClick={() => handleOpenModal('TABLE')}>
                            <FaPlus /> Thêm
                        </button>
                    </div>
                        ...
                        <table className={styles.dataTable}>
                            ...
                            <tbody>
                                {filteredTables.map((t) => (
                                    <tr key={t.tableId}>
                                        <td>{t.name}</td>
                                        <td>{t.personNumber}</td>
                                        <td>{t.position}</td>
                                        <td>
                                            <span
                                                className={t.status === 'Available' ? styles.tagGreen : styles.tagRed}
                                            >
                                                {t.status}
                                            </span>
                                        </td>
                                        <td>
                                            <button
                                                ...
                                                onClick={() => handleViewDetail('TABLE', t)}
                                            >
                                                <FaEye />
                                            </button>
                                            <button
                                                className={styles.iconBtnEdit}
                                                onClick={() => handleOpenModal('TABLE', t)}
                                            >
                                                <FaEdit />
                                            </button>
                                            <button
                                                className={styles.iconBtnDelete}
                                                onClick={() => handleDelete('TABLE', t.tableId)}
                                            >
                                                <FaTrash />
                                            </button>
                                        </td>
                ...
                <div className={styles.section}>
                    <div className={styles.sectionHeader}>
                        <h2 className={styles.title}>
                            <FaUtensils /> Menu ({filteredMenus.length})
                        </h2>
                        <button className={styles.addBtn} onClick={() => handleOpenModal('MENU')}>
                            <FaPlus /> Thêm
                        </button>
                    </div>
                    <div className={styles.listContainer}>
                        <table className={styles.dataTable}>
                            ...
                            <tbody>
                                {filteredMenus.map((m) => (
                                    <tr key={m.foodId}>
                                        <td className={styles.menuName}>{m.name}</td>
                                        <td>
                                            <span className={styles.menuPrice}>{m.price?.toLocaleString()}đ</span>
                                        </td>
                                        <td className={styles.descCell}>{m.description}</td>
                                        <td>
                                            <button
                                                className={styles.iconBtnView}
                                                onClick={() => handleViewDetail('MENU', m)}
                                            >
                                                <FaEye />
                                            </button>
                                            <button
                                                className={styles.iconBtnEdit}
                                                onClick={() => handleOpenModal('MENU', m)}
                                            >
                                                <FaEdit />
                                            </button>
                                            <button
                                                className={styles.iconBtnDelete}
                                                onClick={() => handleDelete('MENU', m.foodId)}
                                            >
                                                <FaTrash />
                                            </button>
                                        </td>
                ...
                <div className={styles.modalOverlay}>
                    <div className={styles.modalContent}>
                        <div className={styles.modalHeader}>
                            <h3>{isView ? 'Chi tiết' : isEdit ? 'Cập nhật' : 'Thêm mới'}</h3>
                        ...
                        <div className={styles.modalFooter}>
                            {isView ? (
                                ...
                                <>
                                    ...
                                    <button className={styles.btnSave} onClick={handleSubmit} disabled={uploading}>
                                        {uploading ? 'Đợi...' : 'Lưu'}
                                    </button>
                                </>
                            )}
