...
const RestaurantMenuManager = () => {
    ...
    useEffect(() => {
        const fetchRestaurants = async () => {
            try {
                const res = await fetch('https://gustoweb.onrender.com/api/admin/AdminRestaurantProfile');
    ...
    if (selectedId) {
        return <RestaurantDetail restaurantId={selectedId} onBack={() => setSelectedId(null)} />;
    }
            ...
            <div className={styles.grid}>
                {filteredRestaurants.length > 0 ? (
                    filteredRestaurants.map((r) => (
                        <div key={r.accountId} className={styles.card} onClick={() => setSelectedId(r.accountId)}>
                            ...
                            <div className={styles.cardBody}>
                                ...
                                <button>👉 Xem Chi Tiết Menu/Bàn</button>
                            </div>
                        </div>
                    ))
                ) : (
