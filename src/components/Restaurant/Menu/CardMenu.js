  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await customFetch(
        `https://localhost:7176/api/RestaurantMenu/deleteMenu/${menu.foodId}`,
        { method: "DELETE" }
      );
    ...
    <div className={styles.cardMenu}>
      ...
      <div className={styles.menuInfo}>
        ...
        {menu.discountPercent > 0 && (
          <p>Giảm giá: {menu.discountPercent}% (Giá cũ: {menu.oldPrice.toLocaleString()} VNĐ)</p>
        )}
        <p>Loại: {menu.type}</p>
      ...
      {isModalOpen && (
        <ModalMenu
          ...
          menu={menu}
          isUpdate
          onSuccess={onSuccess} // Truyền callback vào ModalMenu
        />
      )}
    </div>
  );
