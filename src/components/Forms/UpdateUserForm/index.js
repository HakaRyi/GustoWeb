...
function UpdateUserForm({ initialValues, onProfileReload, isEditing, imgFile }) {
    ...
    const validationSchema = Yup.object({
        ...
        age: Yup.number().nullable().min(18, 'Tuổi phải từ 18 trở lên').max(100, 'Tuổi không được vượt quá 100'),

        address: Yup.string().nullable().min(5, 'Địa chỉ quá ngắn'),

        gender: Yup.string().nullable().oneOf(['Male', 'Female', 'Other'], 'Giới tính không hợp lệ'),

        email: Yup.string().nullable().email('Email không hợp lệ'),

        job: Yup.string().nullable().min(2, 'Tên công việc quá ngắn'),

        description: Yup.string().nullable().min(10, 'Mô tả quá ngắn'),
    });
